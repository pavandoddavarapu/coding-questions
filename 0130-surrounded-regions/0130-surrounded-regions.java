class Solution {
    ...
    public void solve(char[][] board) {
        ...
        for(int i=0;i<board[0].length;i++){
            ...
            if(!visited[n][i] && board[n][i]=='O'){bfs(n,i,visited,board);}
        }
        for(int i=0;i<board.length;i++){
            
            if(!visited[i][0] && board[i][0]=='O'){bfs(i,0,visited,board);}
            int n=board[0].length-1;
            if(!visited[i][n] && board[i][n]=='O'){bfs(i,n,visited,board);}
        }
        for(int i=0;i<board.length;i++){
    ...
    public void bfs(int i1,int j1,boolean[][] visited,char board[][]){
